...
export const ContentProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addContent = async (newPost: Post, newArtwork: Artwork) => {
    try {
        ...
        toast({ variant: 'destructive', title: 'Upload Failed', description: 'Could not save your content to the database.' });
    }
  };
  ...
  const addStoryItem = async (file: File, config: { captionConfigs?: StoryItem['captionConfigs']; mediaConfig?: StoryItem['mediaConfig'] }): Promise<StoryItem> => {
    ...
    const storageRef = ref(storage, `stories/${user.uid || "demo-user"}/${Date.now()}_${file.name}`);
    await uploadBytes(storageRef, file);
    const mediaUrl = await getDownloadURL(storageRef);

    const newItem: Omit<StoryItem, 'id'> = {
        artistId: user.uid || "demo-user",
        mediaUrl,
  ...
  const resharePost = async (postToReshare: Post) => {
    ...
    const currentUserArtist: Artist = {
      ...
      handle: JSON.parse(localStorage.getItem(`userProfile-${user.uid || "demo-user"}`) || '{}').handle || user.email?.split('@')[0] || 'anonymous',
